...
# FINAL IMPROVED app.py (original logic preserved + tick APIs added)
...
def init_mt5_collector():
    ...
    logger.info("MT5 collector started")
...
def get_tick():
    ...
    if tick is None:
        ...
        return jsonify({"error":"no tick"}),404
...
""",(symbol,limit))
        ...
        # oldest → newest (UI table ke liye better)
        rows.reverse()

        return jsonify([dict(r) for r in rows])

    except Exception as e:
        logger.error(f"Tick API error: {e}")
        return jsonify({"error": str(e)}), 500
