...
(async function () {
    ...
    async function pollLiveTick() {
        ...
        candleSeries.update(lastCandleSnap);
    ...
    async function loadTickHistory() {
        ...
        let url = "/api/ticks/history?limit=200";
        ...
        const table = document.getElementById("history-ticks-table");
...
`;
    ...
    const historyBtn = document.getElementById("history-load");

    if (historyBtn) {
        historyBtn.addEventListener("click", loadTickHistory);
    }
    // document
    // .getElementById("history-load")
    // .addEventListener("click",loadTickHistory);
    ...
    async function loadSymbols() {
        ...
        symbols.forEach(sym => {
            ...
            option.value = sym;
            option.textContent = sym.replace(".x", "");

            symbolSelect.appendChild(option);

        });

        currentSymbol = symbols[0]; // default first symbol
    }
    ...
    await loadSymbols();     // 👈 symbols load
    await loadTimeframes();  // 👈 timeframes load

    await fetchAndDraw();    // chart draw
